...
--Individual library coding needs to be updated on lines 11, 18, 34, 41, 43
--Modified to include holds by ptype column for Concord
...
count (distinct CASE WHEN h.pickup_location_code LIKE 'co%' then h.id END) AS "local_holds",
CASE
...
CASE
        ...
        END
        AS "order_copies",
count(distinct h.id) FILTER (WHERE p.ptype_code = '8') AS "total_holds_con_ptype"
...
('m','n','o','e','z','s','$','a','b','d','g','w') AND ((ia.location_code NOT LIKE 'co%' AND ia.itype_code_num not in ('5','21','109','133','160','183')) OR ia.location_code LIKE 'co%')
LEFT JOIN (
...
mv.total_holds_con_ptype AS TotalHoldsConPtype,
        CASE
...
GROUP BY 1, 2, 3, 4, 5, 6, 8, 12, 13, 15
