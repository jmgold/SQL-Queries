...
--Limit to a subject
AND (REPLACE(d.index_entry, ' ', '') LIKE 'prohibitionunitedstates%' OR REPLACE(d.index_entry, ' ', '') LIKE 'cocktails%' OR REPLACE(d.index_entry, ' ', '') LIKE 'brewingamateursmanuals%')
AND NOT REPLACE(d.index_entry, ' ', '') LIKE '%fiction%'
WHERE
b.material_code IN ('a','5') AND b.publish_year >= '2000'
GROUP BY 1,2,3,4) a
ORDER BY RANDOM()
LIMIT 50;