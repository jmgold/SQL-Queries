...
ON
p.record_num = m.record_num and m.record_type_code = 'p' --and m.creation_date_gmt > (localtimestamp - interval '1 month')
WHERE
p.ptype_code IN ('12','312') AND p.home_library_code ~ '^fp3'
AND
((p.home_library_code NOT LIKE '%z' AND p.home_library_code IS NOT NULL) --failed to use pickup location code
OR p.pcode3 != '36' --ma town does not match ptype
...
OR (p.barcode !~ '^\d{14}' AND p.ptype_code = '12') --barcode not 14 digits
