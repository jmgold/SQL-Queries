...
JOIN
sierra_view.patron_record_phone ph
ON
r.id = ph.patron_record_id AND ph.patron_record_phone_type_id = 1
  
WHERE
SUBSTRING(pn.last_name,1,4) || ', ' ||SUBSTRING(pn.first_name,1,3) || COALESCE(' ' || NULLIF(pn.middle_name, ''), '') || ' ' ||ph.phone_number
IN (
	SELECT
	SUBSTRING(n.last_name,1,4) || ', ' ||SUBSTRING(n.first_name,1,3) || COALESCE(' ' || NULLIF(n.middle_name, ''), '') || ' ' ||ph.phone_number AS patron_name
...
ON t1.phone = t2.phone AND t1.patron_record_num = t2.max_record_num 
