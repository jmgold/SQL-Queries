...
COUNT(DISTINCT p.id) FILTER(WHERE ((p.mblock_code != '-') OR (p.owed_amt >= 100))) AS total_block,
COUNT (DISTINCT p.id) FILTER(WHERE ((p.mblock_code != '-') OR (p.owed_amt >= 100)) AND f.charge_code IN ('3','5')) AS total_block_lost_item,
