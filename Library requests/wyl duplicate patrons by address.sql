...
ON
r.id = a.patron_record_id AND a.patron_record_address_type_id = 1
  
WHERE
SUBSTRING(pn.last_name,1,4) || ', ' ||SUBSTRING(pn.first_name,1,3) || COALESCE(' ' || NULLIF(pn.middle_name, ''), '') || ' ' ||a.addr1||' '||a.city 
IN (
	SELECT
	SUBSTRING(n.last_name,1,4) || ', ' ||SUBSTRING(n.first_name,1,3) || COALESCE(' ' || NULLIF(n.middle_name, ''), '') || ' ' ||a.addr1||' '||a.city AS patron_name
...
sierra_view.phrase_entry AS e
ON
e.record_id = r.id AND e.varfield_type_code = 'b'
JOIN
sierra_view.patron_record_address a
ON
r.id = a.patron_record_id AND a.patron_record_address_type_id = 1
...
ON t1.address = t2.address AND t1.patron_record_num = t2.max_record_num 
