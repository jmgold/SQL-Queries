...
SUM(i.year_to_date_checkout_total) AS Total_YTD_Checkouts,
SUM(i.last_year_to_date_checkout_total) AS Total_Last_Year_Checkouts,
...
round(100.0 * (cast(COUNT(i.id) as numeric (12,2)) / (select cast(COUNT (i.id) as numeric (12,2))from sierra_view.item_record i WHERE i.location_code ~ '^wwd' AND i.item_status_code not in ('n','z','e','o','$','w'))), 6)||'%' as relative_item_total,
round(100.0 * (cast(SUM(i.checkout_total) + SUM(i.renewal_total) as numeric (12,2)) / (SELECT cast(SUM(i.checkout_total) + SUM(i.renewal_total) as numeric (12,2)) from sierra_view.item_record i WHERE i.location_code ~ '^wwd' AND i.item_status_code NOT IN ('n','z','e','o','$','w'))), 6)||'%' as relative_circ
FROM
...
WHERE location_code ~ '^wwd'
--location will take the form ^oln, which in this example looks for all locations starting with the string oln.
AND item_status_code NOT IN ('n','z','e','o','$','w')
GROUP BY 1
ORDER BY 1;