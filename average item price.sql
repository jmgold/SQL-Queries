...
AVG(i.price) FILTER(WHERE i.price>'0' and i.price <'1000000') AS "AVG_Price",
MODE() WITHIN GROUP (order by i.price) FILTER(WHERE i.price>'0' and i.price <'1000000') AS "MODE_Price",
MAX(i.price) FILTER(WHERE i.price<'1000000') AS "MAX_Price",
MIN(i.price) FILTER(WHERE i.price>'0') AS "MIN_Price",
((COUNT(i.id)*(AVG(i.price) FILTER(WHERE i.price>'0' and i.price <'1000000')))/(NULLIF(SUM(i.checkout_total),0))) AS "Cost_Per_Circ_By_AVG_Price",
((COUNT(i.id)*(MODE() WITHIN GROUP (order by i.price) FILTER(WHERE i.price>'0' and i.price <'1000000')))/(NULLIF(SUM(i.checkout_total),0))) AS "Cost_Per_Circ_By_Mode_Price"
