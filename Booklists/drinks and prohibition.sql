...
--Limit to a subject
AND ((REPLACE(d.index_entry, ' ', '') LIKE 'cocktails%' OR REPLACE(d.index_entry, ' ', '') LIKE 'brewingamateursmanuals%')
AND NOT REPLACE(d.index_entry, ' ', '') LIKE '%fiction%')
WHERE
b.material_code IN ('a') AND b.publish_year >= '2000'
GROUP BY 1,2,3,4) a
ORDER BY RANDOM()
LIMIT 20)
UNION
...
b.bib_record_id = d.record_id AND d.varfield_type_code = 'd'
--Limit to a subject
AND (((REPLACE(d.index_entry, ' ', '') LIKE 'prohibitionunitedstates%' OR REPLACE(d.index_entry, ' ', '') LIKE 'temperance%' OR REPLACE(d.index_entry, ' ', '') LIKE 'drinkingcustoms%')
AND NOT REPLACE(d.index_entry, ' ', '') LIKE '%fiction%')
OR id2reckey(b.bib_record_id) IN (
'b2917096',
'b1361767',
'b1872052',
'b3037547',
