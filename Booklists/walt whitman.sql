...
ON
b.bib_record_id = d.record_id AND 
d.varfield_type_code = 'd' AND REPLACE(d.index_entry, ' ', '') LIKE '%whitmanwalt18191892%'
WHERE
...
ON b.bib_record_id = r.id AND r.language_code = 'eng'
WHERE
b.material_code = 'a' AND b.publish_year >= '1960' AND REPLACE(SPLIT_PART(SPLIT_PART(b.best_author,' (',1),', ',2),'.','')||' '||SPLIT_PART(b.best_author,', ',1) = 'Walt Whitman'
GROUP BY 1,2,3,4) b
ORDER BY RANDOM()
LIMIT 25);
