...
COUNT(fp.id) FILTER (WHERE fp.payment_status_code != '3') AS fines_paid_count,
...
SELECT
d.all_dates AS DATE,
COUNT(DISTINCT f.id) FILTER (WHERE f.charge_code IN ('1','2','3','4','5','6')) AS count_fines_assessed,
COALESCE(SUM(f.item_charge_amt + f.processing_fee_amt + f.billing_fee_amt) FILTER (WHERE f.charge_code IN ('1','2','3','4','5','6')),'0')::MONEY AS total_fines_assessed,
p.fines_paid_count,
p.fines_paid_total,
