...
COUNT(DISTINCT fc.id) AS fine_count
...
sierra_view.fines_paid fp
ON
i.id = fp.item_record_metadata_id
JOIN
fine_checkout_location fc
ON
'f'||f.id = fc.id OR 'fp'||fp.id = fc.id

WHERE
--limit to book
b.material_code = 'a'
--AND i.location_code ~ '^act'
AND fc.checkout_location ~ '^act'

--figure out limit by checkout location instead
