...
COUNT(DISTINCT p.id) FILTER(WHERE rm.creation_date_gmt::DATE >= '2020-03-01') AS total_new_patrons,
--set date you wish to use for determining if a patron is considered to be new
COUNT(DISTINCT p.id) FILTER(WHERE p.activity_gmt::DATE >= '2020-03-01') AS total_active_patrons,
--set date you wish to use to determine if a patron is considered to be active
ROUND(100.0 * (CAST(COUNT(DISTINCT p.id) FILTER(WHERE p.activity_gmt::DATE >= '2020-03-01') AS NUMERIC (12,2))) / CAST(COUNT(DISTINCT p.id) AS NUMERIC (12,2)), 4) ||'%' AS pct_active,
