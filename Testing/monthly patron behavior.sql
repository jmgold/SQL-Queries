...
SELECT
DISTINCT p.id,
COUNT(DISTINCT t.transaction_gmt::DATE) FILTER (WHERE t.op_code = 'o') AS visits,
COUNT(DISTINCT t.id) FILTER (WHERE t.op_code = 'o') AS checkout_total,
COUNT(DISTINCT t.id) FILTER (WHERE t.op_code = 'o' AND t.application_name != 'sierra') AS self_checkout_total,
COUNT(DISTINCT t.id) FILTER (WHERE t.op_code ~ 'n') AS hold_total,
COUNT(DISTINCT t.id) AS transaction_total,
COUNT(DISTINCT s.location_code) FILTER (WHERE s.location_code NOT IN ('','cmcat','mls')) AS locations_visited,
...
COALESCE(COUNT(DISTINCT p.id) FILTER (WHERE v.id IS NOT NULL),0) AS count_patrons_with_Sierra_transactions,
COALESCE(COUNT(DISTINCT p.id) FILTER (WHERE v.id IS NOT NULL AND v.visits > 0),0) AS count_patrons_with_checkouts,
COUNT(DISTINCT p.id) FILTER(WHERE v.locations_visited > 1) AS patrons_visited_multiple_locations,
COUNT(DISTINCT p.id) FILTER(WHERE v.away_transactions > 0) AS count_patrons_visited_non_home_library,
...
COALESCE(ROUND(AVG(v.checkout_total),2),0) AS avg_monthly_checkouts,
COALESCE(ROUND(SUM(v.checkout_total)/SUM(v.visits),2),0) AS avg_checkouts_per_visit,
COALESCE(SUM(v.hold_total),0) AS hold_total,
COUNT(DISTINCT p.id) FILTER(WHERE COALESCE(v.hold_total, 0) > 0) AS count_patrons_placed_hold,
...
sierra_view.circ_trans t
ON
p.id = t.patron_record_id AND t.transaction_gmt::DATE >= CURRENT_DATE - INTERVAL '1 MONTH'
*/
LEFT JOIN
visits v
ON
