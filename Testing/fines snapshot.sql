...
CASE 
	...
	WHEN f.loanrule_code_num IS NULL THEN 'NA'
	ELSE 'Loan Rule '||f.loanrule_code_num
...
END AS charge_type,
SUM(f.item_charge_amt + f.processing_fee_amt + f.billing_fee_amt)::MONEY AS total_fines,
COUNT(DISTINCT f.patron_record_id) AS total_patrons
...
ON
SUBSTRING(f.charge_location_code FROM 1 FOR 3) = l.code
