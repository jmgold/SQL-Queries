...
FROM (
	SELECT
		...
		COUNT(ir.id) FILTER(WHERE ir.location_code ~ '^lex'/*{{location}}*/) AS local_item_count,
	...
	JOIN (
		SELECT 
			...
			COUNT(DISTINCT h.id) FILTER(WHERE h.pickup_location_code ~ '^lex'/*{{location}}*/) AS local_holds,
			--location will take the form ^oln, which in this example looks for all locations starting with the string oln.
			COUNT(DISTINCT i.id) AS item_count,
			COUNT(DISTINCT ia.id) AS avail_item_count, 
			COUNT(DISTINCT ia.id) FILTER(WHERE ia.location_code ~ '^lex'/*{{location}}*/ AND rmia.creation_date_gmt::DATE >= CURRENT_DATE - INTERVAL '14 days') AS in_process_item_count,
			COUNT(DISTINCT ia.id) FILTER(WHERE ia.location_code ~ '^lex'/*{{location}}*/) AS local_avail_item_count,
			--location will take the form ^oln, which in this example looks for all locations starting with the string oln.
		...
		LEFT JOIN (
		  ...
		  FROM sierra_view.bib_record_order_record_link bro
			...
			JOIN sierra_view.order_record_cmf oc
				ON oc.order_record_id=bro.order_record_id AND oc.location_code ~ '^lex'/*{{location}}*/	
				--location will take the form ^oln, which in this example looks for all locations starting with the string oln.
	...
	WHERE brp.material_code IN ('a','l','u','5'/*{{mat_type}}*/)
	GROUP BY brp.bib_record_id,1, 2, 3, 4, 5, 6, 8, 14, 16, 19
	HAVING mv.local_holds >= 1 --{{min_local_holds}}
...
ORDER BY
	1,2, 
	CASE
		WHEN CAST(local_hold_count AS NUMERIC(12, 2)) / CAST(3 AS NUMERIC(12, 2)) /*{{hold_threshold}}*/  - local_available_item_count - local_order_copies - local_copies_in_process < 0 THEN 0
		ELSE ROUND(CAST(local_hold_count AS NUMERIC(12, 2)) / CAST(3 /*{{hold_threshold}}*/ AS NUMERIC(12, 2)) - local_available_item_count - local_order_copies - local_copies_in_process,1)
	END DESC