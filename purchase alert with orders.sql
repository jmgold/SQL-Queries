--Code shared by Gem Stone Logan over Sierra mailing list on 4/13/17
...
SELECT b.id AS "bib_id", count(distinct h.id) as "hold_count",
CASE
                WHEN count(distinct i.id) IS NULL THEN 0
                ELSE count(distinct i.id)
        END
        AS "item_count",
count(distinct ia.id) as "avail_item_count", max(o1.order_count) AS "order_count",
string_agg(distinct i.location_code, ' ' order by i.location_code desc) as "item_location",
max(o1.order_location) AS "order_location", max(o1.fund_code) AS "fund_code",
CASE
...
LEFT JOIN sierra_view.item_record ia ON ia.id=bri.item_record_id AND ia.item_status_code not in
('m','n','z','s','$','d','8','w','y')
LEFT JOIN (
        ...
        string_agg(concat(fm.code, '(', oc.copies, ')'), ' ' order by fm.code) as "fund_code", bro.bib_record_id AS "bib_id"
...
brp.best_title, brp.best_author, brp.material_code,
max(mv.item_count) as "Total item count", max(mv.avail_item_count) as "available item count",
...
JOIN mvhdholds mv ON mv.bib_id=brp.bib_record_id
