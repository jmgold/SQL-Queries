...
count (distinct CASE WHEN h.pickup_location_code = 'winz' then h.id END) AS "local_holds",
CASE
...
count(distinct CASE WHEN ia.location_code LIKE 'win%' THEN ia.id END) AS "local_avail_item_count",
...
('m','n','o','e','z','s','$','a','b','d','g','w') AND ((ia.location_code NOT LIKE 'win%' AND ia.itype_code_num not in ('5','21','109','133','160','183')) OR ia.location_code LIKE 'win%')
LEFT JOIN (
