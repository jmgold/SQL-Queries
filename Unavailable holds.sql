...
-- ========================================================================
-- Select HOLDS where no items have an available status 
-- Modified from query crated by Brent Searle. Langara College. 2016-10-18
-- ========================================================================
SELECT
  CASE
  ...
  CASE
    WHEN pnam.first_name IS NULL
    THEN pnam.last_name
    ELSE TRIM(pnam.last_name||', '||pnam.first_name)
  END                                                        As "Patron Name",
  CASE
  ...
  CASE
    WHEN hld.status = '0' THEN 'On hold'
    WHEN hld.status = 'b' THEN 'Bib hold ready for pickup'
    WHEN hld.status = 'j' THEN 'Volume hold ready for pickup'
    WHEN hld.status = 'i' THEN 'Item hold ready for pickup'
    WHEN hld.status = 't' THEN 'In transit to pickup location'
    ELSE 'unexpected code '||hld.status
  END                                                        AS "Hold Status",
  locnam.name                                                AS "Pickup Location",
  hld.note                                                   AS "Note"
FROM
    ...
    HAVING
      SUM(
        ...
        CASE
          ...
          WHEN s1_i.item_status_code = 'b' THEN 0 -- Bindery
          WHEN s1_i.item_status_code = 'c' THEN 0 -- Trace
          WHEN s1_i.item_status_code = 'f' THEN 0 -- Media Reformat
          WHEN s1_i.item_status_code = 'k' THEN 0 -- See Dept
