...
New Book list for Cambridge Main Library
*/
SELECT
DISTINCT ON (2)
...
ON
i.item_record_id = ir.id AND ir.location_code LIKE 'cam%' AND ir.icode1 NOT IN ('118','219','241','195')
JOIN sierra_view.record_metadata m
ON
ir.id = m.id AND m.creation_date_gmt >= (localtimestamp - interval '1 month')
GROUP BY 2,3,4,1,5,6
EXCEPT
SELECT
--link to Encore
REPLACE(REPLACE(REPLACE(i.call_number,'|a',''),'[Express] ',''),'[Express View] ','') AS call_number,
...
JOIN sierra_view.item_record ir
ON
i.item_record_id = ir.id AND ir.location_code LIKE 'cam%' AND ir.icode1 NOT IN ('118','181','183','184','185','195','202','203','204','205','209','219','239','241')
JOIN sierra_view.record_metadata m
ON
ir.id = m.id AND m.creation_date_gmt < (localtimestamp - interval '1 month')
GROUP BY 2,3,4,1,5,6
