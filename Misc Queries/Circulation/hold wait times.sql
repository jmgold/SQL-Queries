...
COUNT(DISTINCT i.id) FILTER(WHERE i.location_code ~ '^wyl') AS local_items
...
AND h.pickup_location_code ~ '^wyl' AND h.is_frozen = FALSE AND h.placed_gmt::DATE < CURRENT_DATE
JOIN
...
ON o.id = cmf.order_record_id AND cmf.location_code ~ '^wyl'
