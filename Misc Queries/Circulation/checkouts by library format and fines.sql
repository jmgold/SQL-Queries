...
ROUND((CAST(COUNT(DISTINCT C.id) FILTER(WHERE C.due_gmt::DATE < NOW()::DATE) AS numeric (12,2)) / COUNT(DISTINCT C.id)) * 100,2) AS pct_overdue,
COUNT(DISTINCT c.id) FILTER(WHERE f.charge_code IN ('3','5')) AS total_lost,
ROUND((CAST(COUNT(DISTINCT C.id) FILTER(WHERE f.charge_code IN ('3','5')) AS numeric (12,2)) / COUNT(DISTINCT C.id)) * 100,2) AS pct_lost,
...
ROUND((CAST(COUNT(DISTINCT p.id) FILTER(WHERE C.due_gmt::DATE < NOW()::DATE) AS numeric (12,2)) / COUNT(DISTINCT p.id)) * 100,2) AS pct_with_overdue,
COUNT(DISTINCT p.id) FILTER(WHERE f.charge_code IN ('3','5')) AS total_patrons_with_lost,
ROUND((CAST(COUNT(DISTINCT p.id) FILTER(WHERE f.charge_code IN ('3','5')) AS numeric (12,2)) / COUNT(DISTINCT p.id)) * 100,2) AS pct_with_lost
