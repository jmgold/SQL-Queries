...
FROM(
  ...
  JOIN sierra_view.subfield sent1
    ON o.id = sent1.record_id
	   AND sent1.field_type_code = 'b'
		AND sent1.tag = 'b'
		AND TO_DATE(SUBSTRING(sent1.content, '\d{2}\-\d{2}\-\d{4}'),'MM-DD-YYYY') IS NOT NULL --AND sent1.occ_num = '0'
  JOIN sierra_view.subfield sent2
    ON o.id = sent2.record_id
	   AND sent2.field_type_code = 'b'
	   AND sent2.tag = 'b'
	   AND TO_DATE(SUBSTRING(sent2.content, '\d{2}\-\d{2}\-\d{4}'),'MM-DD-YYYY') IS NOT NULL
  ...
  WHERE o.accounting_unit_code_num = 4
...
WHERE inner_query.created_date BETWEEN '2025-07-01' AND '2025-10-31'