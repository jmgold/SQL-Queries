    ...
    with open(csv_file, "w", encoding="utf-8", newline="") as tempFile:
        ...
        while offset < 6000000:
            ...
            SELECT
              ...
              --filtering out locations used for digital collections, training, ILL, etc...
...
def main():
    ...
    SELECT
      ...
      --filling in MA as state where it can be assumed but data entry for state field is bad
      COALESCE(CASE
    ...
    --location join accounts for colleges in Minuteman with differing location and ptype naming conventions from publics  
    ...
    WITH copies_ordered AS (
      ...
      */  
      JOIN sierra_view.order_record_cmf cmf
        ...
        --limit to orders that are still on order or have only been partially received
        AND o.order_status_code IN ('o','q')
        --if you use multi selection groups in ordering you may have multi rows in the cmf table that can lead to double counts
