...
count (distinct CASE WHEN h.pickup_location_code = 'mwyz' then h.id END) AS "local_holds",
CASE
...
count(distinct CASE WHEN ia.location_code LIKE 'mwy%' THEN ia.id END) AS "local_avail_item_count",
...
('-','t','p','!') AND ((ia.location_code NOT LIKE 'mwy%' AND ia.itype_code_num not in ('5','21','109','133','160','183')) OR ia.location_code LIKE 'mwy%')
LEFT JOIN (
