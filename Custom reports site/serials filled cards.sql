...
FROM (
  ...
  FROM sierra_view.holding_record_box b
  JOIN sierra_view.holding_record_cardlink cl
    ON b.holding_record_cardlink_id = cl.id
  JOIN sierra_view.holding_record_card card
    ON cl.holding_record_card_id = card.id
  JOIN sierra_view.holding_record h
    ON card.holding_record_id = h.id
  JOIN sierra_view.bib_record_holding_record_link l
    ON h.id = l.holding_record_id
  ...
  JOIN sierra_view.user_defined_scode2_myuser s
    ON h.scode2 = s.code

  --Excludes cards marked as filled
  WHERE card.status_code != 'F'
	 AND hl.location_code ~ {{location}}
