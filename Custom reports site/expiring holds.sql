...
Takes two filters, pickup locations and number of days before holds expire
*/
...
FROM (
  SELECT
    ...
    COALESCE((
	   SELECT
        pp.phone_number
      FROM sierra_view.patron_record_phone pp
      WHERE h.patron_record_id = pp.patron_record_id
  ...
  JOIN sierra_view.varfield barcode
    ON h.patron_record_id = barcode.record_id
	 AND barcode.varfield_type_code = 'b'
  LEFT JOIN sierra_view.varfield email
    ON h.patron_record_id = email.record_id
