...
SELECT
  *,
  '' AS "ITEM DATA",
  '' AS "https://sic.minlib.net/reports/75"
FROM (
  SELECT
    DISTINCT i.id,
    rm.record_type_code,
  ...
  JOIN sierra_view.agency_property_myuser a
    ON i.agency_code_num = a.code
  JOIN sierra_view.statistic_group_myuser AS so
    ON i.checkout_statistic_group_code_num = so.code
  JOIN sierra_view.statistic_group_myuser AS si
    ON i.checkin_statistics_group_code_num = si.code
