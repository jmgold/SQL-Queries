...
Search for any cancelled holds on a given item given a barcode and cancelled date
*/

SELECT
  *,
  '' AS "CANCELLED HOLD LOOKUP",
  '' AS "https://sic.minlib.net/reports/111"
FROM (
  SELECT
  ...
  WHERE ip.barcode = TRIM({{barcode}})
    AND h.removed_gmt::DATE = {{date_cleared}}
)a