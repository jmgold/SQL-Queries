  ...
  SELECT
    ...
    */
    ...
    ROUND(100 * (CAST(COUNT(C.id) FILTER(WHERE C.op_code = 'o') AS NUMERIC (12,2)) / (
	   SELECT
		  CAST(COUNT (C.id) as numeric (12,2))
		...
		WHERE CASE 
          --location using the form ^act in order to reuse an existing filter
	       ...
	       WHEN  {{location}} = '^fpl' THEN C.stat_group_code_num BETWEEN '360' AND '369' OR C.stat_group_code_num = '961'
	       WHEN  {{location}} = '^fp3' THEN C.stat_group_code_num = '373'
	       WHEN  {{location}} = '^fp2' THEN C.stat_group_code_num BETWEEN '370' AND '379'
	       WHEN  {{location}} = '^fp' THEN C.stat_group_code_num BETWEEN '360' AND '379' OR C.stat_group_code_num = '961'
	       ...
	       WHEN  {{location}} = '^med' THEN C.stat_group_code_num BETWEEN '480' AND '489' OR C.stat_group_code_num = '983'
	       ...
	       WHEN  {{location}} = '^sud' THEN C.stat_group_code_num BETWEEN '690' AND '699' OR C.stat_group_code_num = '971'
	       WHEN  {{location}} = '^wlm' THEN C.stat_group_code_num BETWEEN '700' AND '709' OR C.stat_group_code_num = '993'
	       WHEN  {{location}} = '^wl2' THEN C.stat_group_code_num = '981'
	       WHEN  {{location}} = '^wl' THEN C.stat_group_code_num BETWEEN '700' AND '709' OR C.stat_group_code_num IN ('993','981')
	       ...
	       WHEN  {{location}} = '^wel' THEN C.stat_group_code_num BETWEEN '750' AND '759' OR C.stat_group_code_num = '951'
	       ...
	       WHEN  {{location}} = '^we' THEN C.stat_group_code_num BETWEEN '750' AND '779' OR C.stat_group_code_num = '951'
	       WHEN  {{location}} = '^win' THEN C.stat_group_code_num BETWEEN '780' AND '789'
	       WHEN  {{location}} = '^wob' THEN C.stat_group_code_num BETWEEN '790' AND '799' OR C.stat_group_code_num = '973'
