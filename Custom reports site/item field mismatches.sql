...
FROM (
  ...
  JOIN item_group ig
    ON CASE
      WHEN '{{field_one}}' = 'i.location_code' THEN (i.location_code::VARCHAR = ig.field_one::VARCHAR)
	 ...
	 END
    AND CASE
		WHEN '{{field_two}}' = 'i.location_code' THEN (i.location_code::VARCHAR = ig.field_two::VARCHAR)
  ...
  ORDER BY ig.field_one, ig.field_two
)a