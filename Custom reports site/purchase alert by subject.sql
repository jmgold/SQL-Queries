...
On Demand Purchase Alert limited by subject
*/

SELECT
  *,
  '' AS "PURCHASE ALERT BY SUBJECT",
  '' AS "https://sic.minlib.net/reports/89"
FROM (
  SELECT
  ...
  JOIN (
    ...
    WHERE REPLACE(p.index_entry, ' ', '') LIKE TRANSLATE(REGEXP_REPLACE(LOWER('{{subject}}'),'\|[a-z]','','g'), ' .,-()', '')
