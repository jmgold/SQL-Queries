...
SELECT
  *,
  '' AS "ORDER DATA",
  '' AS "https://sic.minlib.net/reports/78"
...
FROM (
  ...
  JOIN sierra_view.accounting_unit a
    ON o.accounting_unit_code_num = {{accounting_unit}}
  ...
  JOIN sierra_view.order_type_property_myuser ot
    ON o.order_type_code = ot.code
  JOIN sierra_view.acq_type_property_myuser acqt
    ON o.acq_type_code = acqt.code
  JOIN sierra_view.form_property_myuser form
    ON o.form_code = form.code
  JOIN sierra_view.order_note_property_myuser note
    ON o.order_note_code = note.code
  JOIN sierra_view.order_status_property_myuser os
    ON o.order_status_code = os.code
  ...
  JOIN sierra_view.fund_master fm
    ON cmf.fund_code::NUMERIC = fm.code_num
