...
WITH available_titles AS (
  --bibs with available copies and holds
  SELECT
...
FROM (
  ...
  JOIN sierra_view.record_metadata rmh
    ON h.record_id = rmh.id
  ...
  LEFT JOIN available_titles a
    ON b.bib_record_id = a.bib_record_id

  WHERE h.pickup_location_code ~ '{{location}}' 
  --location will take the form ^oln, which in this example looks for all locations starting with the string oln.
    AND a.bib_record_id IS NULL
