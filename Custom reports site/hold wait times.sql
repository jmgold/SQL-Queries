...
bib_hold AS (
  ...
  JOIN sierra_view.bib_record_item_record_link l
    ON h.record_id = l.bib_record_id
	 ...
	 AND h.on_holdshelf_gmt IS NOT NULL
  JOIN sierra_view.record_metadata rm
...
local_info AS (
  SELECT
    ...
    COUNT(DISTINCT i.id) FILTER(WHERE i.location_code ~ {{location}}) AS local_items
  ...
  WHERE h.pickup_location_code ~ {{location}}
    AND h.is_frozen = FALSE
	 AND h.placed_gmt::DATE < CURRENT_DATE
  GROUP BY 1
),

--Gather count of copies on order at a location for each title
local_orders AS (
  SELECT 
    l.bib_record_id,
    SUM(cmf.copies) AS copies

  FROM sierra_view.bib_record_order_record_link l
...
FROM (
  SELECT
    ...
    l.local_items,
    COALESCE(o.copies,0) AS copies_on_order

  FROM bib_hold bh
  JOIN cat_date cd
    ON bh.bib_record_id = cd.bib_record_id
  JOIN sierra_view.bib_record_property b
    ON bh.bib_record_id = b.bib_record_id
  JOIN sierra_view.material_property_myuser mat
    ON b.material_code = mat.code
  JOIN local_info l
    ON b.bib_record_id = l.bib_record_id
  LEFT JOIN local_orders o
    ON b.bib_record_id = o.bib_record_id

  GROUP BY 1,2,3,4,6,8,9
  HAVING COUNT(bh.id) FILTER (WHERE bh.status = 'active') > 0
    AND AVG(
	   CASE
	     WHEN cd.cat_date <= bh.placed_gmt::DATE THEN bh.end_date - bh.placed_gmt::DATE
	     ELSE bh.end_date - cd.cat_date
