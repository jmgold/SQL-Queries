...
FROM
sierra_view.holding_record_location loc
...
ON
h.accounting_unit_code_num = a.code_num
JOIN
...
ON
h.vendor_code = v.code AND h.accounting_unit_code_num = v.accounting_unit_code_num
JOIN
sierra_view.varfield vn
ON
v.id = vn.record_id AND vn.varfield_type_code = 't'
JOIN
sierra_view.user_defined_scode1_myuser s1
ON
h.scode1 = s1.code
JOIN
sierra_view.user_defined_scode2_myuser s2
ON
h.scode2 = s2.code
JOIN
sierra_view.user_defined_scode3_myuser s3
ON
h.scode3 = s3.code
JOIN
sierra_view.user_defined_scode4_myuser s4
ON
h.scode4 = s4.code
JOIN
sierra_view.holding_record_location loc
ON
h.id = loc.holding_record_id
JOIN
sierra_view.location_myuser l
ON
loc.location_code = l.code
