...
FROM (
  SELECT
    ...
    COUNT(C.id) FILTER (WHERE C.stat_group_code_num BETWEEN '360' AND '379'OR C.stat_group_code_num = '961') AS "FRAMINGHAM",
    ...
    COUNT(C.id) FILTER (WHERE C.stat_group_code_num BETWEEN '480' AND '489'OR C.stat_group_code_num = '983') AS "MEDFORD",
    ...
    COUNT(C.id) FILTER (WHERE C.stat_group_code_num BETWEEN '690' AND '699'OR C.stat_group_code_num = '971') AS "SUDBURY",
    ...
    COUNT(C.id) FILTER (WHERE C.stat_group_code_num BETWEEN '750' AND '779' OR C.stat_group_code_num = '951') AS "WELLESLEY",
    COUNT(C.id) FILTER (WHERE C.stat_group_code_num BETWEEN '800' AND '809') AS "WESTON",
    COUNT(C.id) FILTER (WHERE C.stat_group_code_num BETWEEN '810' AND '829' OR C.stat_group_code_num = '831') AS "WESTWOOD",
    COUNT(C.id) FILTER (WHERE C.stat_group_code_num BETWEEN '780' AND '789') AS "WINCHESTER",
    COUNT(C.id) FILTER (WHERE C.stat_group_code_num BETWEEN '790' AND '799'OR C.stat_group_code_num = '973') AS "WOBURN"
