...
SELECT
  ...
  invoice_total,
  '' AS "X ORDER FINDER",
  '' AS "https://sic.minlib.net/reports/38"
FROM (
  ...
  JOIN sierra_view.fund_master f
    ON l.fund_code::INTEGER = f.code_num
	 AND a.id = f.accounting_unit_id
  JOIN sierra_view.fund_myuser n
    ON f.id = n.fund_master_id

  WHERE l.order_record_metadata_id IS NULL
