...
FROM (
  ...
  JOIN sierra_view.holding_record h
    ON l.holding_record_id = h.id
  JOIN sierra_view.holding_record_location hl
    ON h.id = hl.holding_record_id
  JOIN sierra_view.holding_record_card hc
    ON h.id = hc.holding_record_id
  JOIN sierra_view.holding_record_cardlink cl
    ON hc.id = cl.holding_record_card_id
  JOIN sierra_view.holding_record_box hb
    ON cl.id = hb.holding_record_cardlink_id
	 AND box_status_code = 'A'
  
  --location will take the form ^oln, which in this example looks for all locations starting with the string oln.
  WHERE hl.location_code ~ '{{location}}'
  GROUP BY 1,2,4,5,6
  ORDER BY 2
) t1
