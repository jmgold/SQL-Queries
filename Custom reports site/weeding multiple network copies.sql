...
and where the recent local turnover is less than the network turnover
*/

WITH bib_list AS (
  SELECT
    l.bib_record_id,
    (SUM(i.year_to_date_checkout_total) + SUM(i.last_year_to_date_checkout_total)) AS local_checkout_total,
...
FROM (
  SELECT
    ...
    (SUM(i.year_to_date_checkout_total) + SUM(i.last_year_to_date_checkout_total)) AS non_local_checkout_total,
    COUNT(i.id) AS non_local_copies,
    ROUND(CAST((SUM(i.year_to_date_checkout_total) + SUM(i.last_year_to_date_checkout_total)) AS NUMERIC (12,2))/CAST(COUNT(i.id) AS NUMERIC (12,2)), 2) AS non_local_turnover,
    bl.item_numbers,
    bl.call_numbers,
    bl.scat_codes,
    bl.itypes
...
ORDER BY a.non_local_turnover - a.local_turnover DESC,
  a.non_local_copies - a.local_copies DESC