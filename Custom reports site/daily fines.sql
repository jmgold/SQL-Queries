...
WITH paid_per_day AS (
  ...
  FULL OUTER JOIN (
    ...
    FROM GENERATE_SERIES(0,35,1) AS offs
	 ) d
    ON fp.paid_date_gmt::DATE = d.all_dates

  WHERE fp.paid_date_gmt >= NOW()::DATE - INTERVAL '1 month'
...
FROM (
  ...
  FROM (
    ...
    LEFT JOIN paid_per_day p
      ON d.all_dates = p.paid_date
    LEFT JOIN sierra_view.fine f
      ON d.all_dates = f.assessed_gmt::DATE

  WHERE d.all_dates >= NOW()::DATE - INTERVAL '1 month'
    AND CASE 
